...
public class http_getcollectionint {
    public static  String getText(int mid) {
        try {
            ...
            String u = ("http://8.140.3.158:81/collection/search/"+ mid);//keyword是查询关键字
            URL url = new URL(u);
            ...
            collectioninfo collectioninfo = gson.fromJson(buffer.toString(), collectioninfo.class);

            return collectioninfo.getCintroduce();//返回名字 同理可以返回对象后使用getName..等方法获得别的信息 然后放到布局文件就可以了
