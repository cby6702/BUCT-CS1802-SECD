...
public class http_getmuseummid {
    public static  int getText(String keyword) {
        try {
            ...
            return museumInfo.getMid();//返回名字 同理可以返回对象后使用getName..等方法获得别的信息 然后放到布局文件就可以了
    ...
    public static  String getpicture(String keyword) {
        try {
            // URL url = new URL("http://openapi.tuling123.com/openapi/api/v2" );
            //URL url = new URL(https://api.ownthink.com/bot")
            String u = "http://8.140.3.158:81/museum/searchName/"+ URLEncoder.encode(keyword,"utf-8");//keyword是查询关键字
            ...
            return museumInfo.getPicture();//返回名字 同理可以返回对象后使用getName..等方法获得别的信息 然后放到布局文件就可以了
