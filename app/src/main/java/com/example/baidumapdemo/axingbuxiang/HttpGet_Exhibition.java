...
public class HttpGet_Exhibition {
    public  static List<Exhibition> getText(String ename) {
        ...
        try {
            ...
            Type listType = new TypeToken<List<Exhibition>>() {}.getType();//java通过反射获取对象类型
            List<Exhibition> ExhibitionList = gson.fromJson(buffer.toString(), listType);

            return ExhibitionList ;//返回名字 同理可以返回对象后使用getName..等方法获得别的信息 然后放到布局文件就可以了
